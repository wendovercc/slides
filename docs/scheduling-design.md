...
```json
"audience": {
  "section": "senior",       // "senior" | "women" | "junior" | "social" | "all"
...
All teams have `play_cricket_team_id`, `cs365_team_name` and `section`
(`senior` | `women` | `junior`). **Still needed:** `home_location` per team.
...
| `content/teams.json` | Add `home_location` per team (`section` ✓ done) | Partially done |
...
- **`section` derivation**: settled — each team declares its own `section` in
  `teams.json`, and `infer_section()` falls back to the old team-ID-prefix guess
  (`u` → junior) only for a team that doesn't. `social` is the one section no team
  belongs to: it is the audience of the members bar, which is what a social member
  joins for, so bar-time slides name `social` in their `show_when`.
